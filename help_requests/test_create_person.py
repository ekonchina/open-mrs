...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"names": [], "gender": "M"},
        # missing givenName
        {"names": [{"familyName": "Doe"}], "gender": "M"},
        # missing familyName
        {"names": [{"givenName": "John"}], "gender": "M"},
        # missing gender
        {"names": [{"givenName": "John", "familyName": "Doe"}]},
        # invalid gender
        {"names": [{"givenName": "John", "familyName": "Doe"}], "gender": "X"},
        # invalid age type
        {"names": [{"givenName": "John", "familyName": "Doe"}], "gender": "M", "age": "30"},
        # invalid birthdate format
        {"names": [{"givenName": "John", "familyName": "Doe"}], "gender": "M", "birthdate": "15-01-1990"},
        # names wrong type
        {"names": "not-a-list", "gender": "M"},
    ],
)
def test_create_person_negative_validation(payload):
    resp = api_post_person(payload)

    # OpenMRS иногда отдаёт 400, иногда (в зависимости от версии/валидации) может прилететь 500.
    assert resp.status_code >= 400, f"Expected error (>=400), got {resp.status_code}\n{resp.text}"

    # Пытаемся разобрать JSON-ошибку
    try:
        data = resp.json()
    except Exception:
        pytest.fail(f"Expected JSON error body, got non-JSON: {resp.text}")

    print("NEGATIVE RESPONSE JSON:\n", pretty(data))

    # Универсальная проверка структуры ошибки: должны быть хоть какие-то признаки ошибки
    assert isinstance(data, dict), f"Error body must be dict; got {type(data)}"
    assert any(k in data for k in ("error", "errors", "message", "exception", "localizedMessage")), (
        f"Unexpected error response structure:\n{pretty(data)}"
    )
